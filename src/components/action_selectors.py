...
class EpsilonGreedyActionSelector():
    ...
    def __init__(self, args):
        ...
        # 添加高斯噪声相关参数
        self.use_gaussian_noise = getattr(args, "use_gaussian_exploration", True)
        self.noise_scale = getattr(args, "gaussian_noise_scale", 0.2)
    ...
    def select_action(self, agent_inputs, avail_actions, t_env, test_mode=False):
        ...
        if self.use_gaussian_noise and not test_mode:
            # 使用高斯噪声进行探索
            exploration_actions = self._gaussian_exploration(masked_q_values, avail_actions)
        else:
            # random_actions = Categorical(avail_actions.float()).sample().long()
            exploration_actions = Categorical(avail_actions.cpu().float()).sample().long().to(avail_actions.device)

        # # random_actions = Categorical(avail_actions.float()).sample().long()
        # random_actions = Categorical(avail_actions.cpu().float()).sample().long().to(avail_actions.device)

        picked_actions = pick_random * exploration_actions + (1 - pick_random) * masked_q_values.max(dim=2)[1]
        return picked_actions

    def _gaussian_exploration(self, q_values, avail_actions):
        """使用高斯噪声进行探索"""
        # 计算噪声尺度
        current_noise_scale = self.noise_scale
            
        # 添加高斯噪声到Q值
        noise = th.randn_like(q_values, device=q_values.device) * current_noise_scale
        noisy_q_values = q_values + noise
        
        # 确保无效动作仍然不可选
        noisy_q_values[avail_actions == 0] = -float("inf")
        
        # 选择噪声扰动后的最大Q值动作
        return noisy_q_values.max(dim=2)[1]
