...
class QattenMixer(nn.Module):
    def __init__(self, args):
        ...
        # 改进1: 将固定的缩放因子改为可学习参数
        self.scaling_factor = nn.Parameter(th.tensor(np.sqrt(args.n_query_embedding_layer2), dtype=th.float32))

        # 改进2: 在head embedding layer中添加dropout和layer norm
        self.head_embedding_layer = nn.Sequential(
            ...
            nn.LayerNorm(self.n_head_embedding_layer2)  # 添加LayerNorm稳定训练
        )
    ...
    def forward(self, agent_qs, states):
        ...
        for i in range(self.n_attention_head):
            ...
            # 改进1: 使用可学习的缩放因子替代固定的scaled_product_value
            raw_lambda = th.matmul(state_embedding, u_embedding) / th.abs(self.scaling_factor)
        ...
        if self.args.weighted_head == True:
