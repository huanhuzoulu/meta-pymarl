...
class EpisodeRunner:
    ...
    def _log(self, returns, stats, prefix):
        ...
        # 在evaluate模式下，记录test_nepisode回合的胜率情况
        if self.args.evaluate and prefix == "test_":
            # 记录胜率信息
            if "battle_won" in stats:
                win_rate = stats["battle_won"] / stats["n_episodes"]
                self.logger.log_stat(prefix + "win_rate", win_rate, self.t_env)
                print(f"Evaluate mode - Win Rate over {stats['n_episodes']} episodes: {win_rate:.3f}")      
